...
const server = new McpServer(
  { name: "vpsnet", version: "1.0.0" },
  {
    instructions: [
      ...
      "1. Read YOUR local machine's public key from ~/.ssh/id_rsa.pub or ~/.ssh/id_ed25519.pub",
      "2. Upload it via create_ssh_key(name, public_key) — name it descriptively e.g. 'claude-code-key'",
      "3. Deploy it to the VPS via deploy_ssh_key(orderNo, ssh_key_id) OR pass sshKey when ordering a new VPS",
      "4. SSH directly using your environment's terminal: ssh -o StrictHostKeyChecking=no root@<vps_ip>",
      "5. Now you can run any commands inside the VPS (apt install, configure services, deploy apps, etc.)",
      "",
      "### Important SSH notes:",
      ...
      "- Only fall back to rootPassword if the local machine has no SSH key (e.g. Windows without OpenSSH).",
      "- If your environment has no terminal/bash access at all, consider using a dedicated SSH MCP server for connectivity.",
      "",
      "## Plan changes",
