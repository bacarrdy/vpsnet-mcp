...
const server = new McpServer(
  { name: "vpsnet", version: "1.0.0" },
  {
    instructions: [
      ...
      "When ordering or accessing a VPS, PREFER using sshKey over rootPassword — unless on Windows without an SSH client.",
      "To access a VPS via SSH, you MUST use YOUR OWN local machine's SSH key:",
      "1. Read the local machine's public key from ~/.ssh/id_rsa.pub or ~/.ssh/id_ed25519.pub",
      "2. Upload it via create_ssh_key(name, public_key) — name it descriptively e.g. 'claude-code-local'",
      "3. Deploy it to the VPS via deploy_ssh_key(orderNo, ssh_key_id) OR pass sshKey when ordering",
      "4. Then SSH directly: ssh root@<vps_ip>",
      "NEVER use someone else's SSH key — always read from the local machine where you are running.",
      "Only fall back to rootPassword if the local machine has no SSH key and no SSH client (e.g. Windows without OpenSSH).",
      "Note: order_service uses 'sshKey' (camelCase) but deploy_ssh_key uses 'ssh_key' (snake_case) — different field names!",
      "To deploy software inside a VPS: first deploy an SSH key to the VPS, then access it directly via SSH. If no SSH client is available (e.g. Windows without OpenSSH), use rootPassword instead and connect via the platform's console or other remote access.",
      "",
      "## Plan changes",
